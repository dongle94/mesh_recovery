...
class HybrIKTask(task.Job):
    ...
    def init(self):
        ...
        if self.cfg.hybrikx is True:
            from core.hybrik.utils.presets.simple_transform_3d_smplx import SimpleTransform3DSMPLX
            transform = SimpleTransform3DSMPLX
        else:
            from core.hybrik.utils.presets.simple_transform_3d_smpl_cam import SimpleTransform3DSMPLCam
            transform = SimpleTransform3DSMPLCam

        init_logger(self.cfg)

        if self.cfg.hybrikx is True:
            cfg_file = './core/hybrik/configs/256x192_hrnet_rle_smplx_kid.yaml'
        else:
            cfg_file = './core/hybrik/configs/256x192_adam_lr1e-3-hrw48_cam_2x_w_pw3d_3dhp.yaml'
    ...
    def process(self, item=None):
        ...
        for tight_box in bboxes:
            ...
            # HybrIKX: (71, 3) / HybrIK: (29, 3)
            uv_joints = pose_output.pred_uvd_jts.reshape(-1, 3)[:, :2]
            pts = uv_joints * bbox_xywh[2]
        ...
        if self._is_face_sent is False:
            if self.cfg.hybrikx is True:
                item['smpl_faces'] = torch.from_numpy(self.hybrik_model.smplx_layer.faces.astype(np.int32))
            else:
                item['smpl_faces'] = torch.from_numpy(self.hybrik_model.smpl.faces.astype(np.int32))
...
class PostHybrIKTask(task.Job):
    ...
    def process(self, item=None):
        ...
        if self.smpl_faces is None:
            smpl_faces = item['smpl_faces']
            self.smpl_faces = smpl_faces.cuda() if self.cfg.device == 'cuda' else smpl_faces.cpu()
        ...
        for idx, ret in enumerate(rets):
            ...
            # TODO: modify draw heatmap
            if self.cfg.hybrik_draw_heatmap is True:
                bh = bboxes[idx][3] - bboxes[idx][1]
                valid_idx = [4, 18, 19, 20, 21]
                color_dict = {4: 'red', 18: 'green', 19: 'green', 20: 'blue', 21: 'red'}
                frame = vis.draw_heatmap(frame, pts, valid_idx, colors=color_dict, mask=valid_mask, height=bh)
                b_frame = vis.draw_heatmap(b_frame, pts, valid_idx, colors=color_dict, mask=valid_mask, height=bh)
