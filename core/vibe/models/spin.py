...
from core.vibe.utils.geometry import rotation_matrix_to_angle_axis, rot6d_to_rotmat
from core.vibe.models.smpl import SMPL
from core.vibe.constants import H36M_TO_J14
...
class HMR(nn.Module):
    ...
    def __init__(self, block, layers, smpl_mean_params):
        ...
        self.smpl = SMPL(
            os.path.join(cfg.vibe_data_dir, 'J_regressor_extra.npy'),
            cfg.vibe_data_dir,
            batch_size=64,
            create_transl=False
        ).to('cpu')
...
def get_pretrained_hmr(vibe_data_dir):
    device = 'cuda'
    model = hmr().to(device)
    checkpoint = torch.load(os.path.join(vibe_data_dir, 'spin_model_checkpoint.pth.tar'))
